<script lang="ls">
import {getLableByPage,deleteLable,addOrUpdateLable} from "@/api/lable";

export default {
  methods: {
    confirmEvent(id){
      return new Promise(()=>{
        deleteLable(id).then(response=>{
    ...
    async addLable() {
      this.$prompt('请输入标签名称', '添加标签', {
        ...
        addOrUpdateLable({name:value}).then(response=>{
          this.getAll(this.pageNum)
        })
      }).catch(() => {
        this.$message({
    ...
    updateLable(id,name) {
      this.$prompt('请输入标签名称', '修改标签', {
        ...
        addOrUpdateLable({id:id,name:value}).then(response=>{
          this.getAll(this.pageNum)
        })
      }).catch(() => {
        this.$message({
    ...
    getAll(val) {
      ...
      return new Promise(() => {
        getLableByPage(this.param).then((response) => {
          const { data } = response;
          console.log(data);
          if (!data) {
            return reject("网络异常.");
          ...
          >
            <template slot-scope="scope">
              <el-button
                type="primary"
                size="mini"
                @click="updateLable(scope.row.id, scope.row.name)"
                >修改</el-button
              >
              <el-popconfirm
                title="确定要删除吗？"
